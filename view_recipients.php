...
if(!$conn)  
    ...
    $checkdata="SELECT * FROM recipient_log ";
    $search=mysqli_query($conn,$checkdata);
    if(mysqli_num_rows($search)>0){
        while($row=mysqli_fetch_assoc($search)){
            echo "<center style='color: white;'>Name: <b>{$row['recipient_name']}</b> Age: <b>{$row['recipient_age']}</b>  Gender: <b>{$row['recipient_gender']}</b>  Blood Type: <b>{$row['recipient_blood']}</b>  City: <b>{$row['recipient_city']}</b></center>";
            echo "<br>";
        }
    }
}
